...
| Ingestion ownership | Fluentd, Firehose, and Lambda shared transport behavior | OSIS owns the managed runtime; the platform owns its declarative pipeline and schema |
...
| Schema | Dynamic fields could become searchable without a review boundary | The core index template uses `dynamic: false`; unknown fields remain in `_source` until schema review maps them |
...
This change is about responsibility placement rather than dismissing the earlier architecture. AWS operates the managed ingestion runtime; platform engineering retains occurrence-time semantics, parsing outcomes, schema and template governance, archive and search retention, access, monitoring, failure classification, and replay.
