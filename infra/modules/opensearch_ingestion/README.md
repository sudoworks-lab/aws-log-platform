...
The sink uses `index_type: management_disabled`, writes only to a pre-created index, and routes individual documents rejected by OpenSearch to a separate S3 sink DLQ. It does not install templates or manage mappings; the `opensearch_serverless` module owns the AOSS index schema. The pipeline uses the default in-memory buffer because the raw S3 archive and SQS queue are the explicit durability boundary for this design.
